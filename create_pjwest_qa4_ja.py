...
OUT = "/Users/treqd/Desktop/Cursor/西友PMI/PJWest_WMS_Sinops_QA_ReSummary_20260902_QA4_JA.docx"
...
def build():
    ...
    add_text(doc, "最新QAおよび定例会資料（2026-09-02）に基づく設計・開発判断", size=13, color=MUTED, after=12)
    add_text(doc, "日付: 2026-09-02", size=10.5, color=MUTED, after=2)
    ...
    add_callout(doc, "総合判断：QA回答は全体として収束しているが、「完了」は実装仕様の完全確定を意味しない。WMSとSinopsは、残課題および外部依存を分けて段階的に設計・開発へ進める。9/8のPhase2承認に向け、連携経路と移行前提を先に固定する。")
    ...
    doc.add_heading("7. 2026年9月2日定例会の追加反映", level=1)
    add_callout(doc, "定例会更新：自動補充I/Fはマッピング・GAP分析完了後、9/8ステコミでPhase2（詳細設計・開発）のコスト承認を依頼予定。WMSはTREとのI/F要件定義を継続し、日立による必要ドキュメント整理とHimalaya並行稼働案の検討を進める。", fill="FFF6E5", bar="A67C00")
    add_bullet(doc, "自動補充・棚割の連携フローは、TRMD基幹を起点にGCSへ出力し、①集計SV→GCS、②GCS→西友DWH、③DWH／GCS→MDSCM系・BO系システムの3層で整理する。DWHを経由しないIFは個別経路として管理する。")
    add_bullet(doc, "自動補充はSinops-R側21本のIF一覧で、マスタ5本、販売実績3本、在庫2本、仕入2本、発注1本、新商品在庫1本、棚割1本を確認。Sinops-W側はマスタ4本、発注2本を継続整理中。")
    add_bullet(doc, "2026年10月の外部連携テスト時点で、テスト環境への店番・仕入先コードの桁数拡張後データ登録を完了させる。店番は2026年11月まで、仕入先コード登録は2026年9～11月、商品Master移行は2026年12月～2027年2月を前提とする。")
    add_bullet(doc, "店舗コンバージョンは2027年5～6月にパイロット、同年7～9月に店舗単位で順次切替、10月以降に安定化・本格運用へ移行する。自動補充、棚割、WMSは既存システムを継続利用し、TRMD基幹とのIFを構築する。")
    add_bullet(doc, "移行方式は店舗CVに合わせて発注・EDI・会計を段階的に切替える案が有力。未CV／CV済店舗の過渡期ケース、Shinise・WMS・会計・リベートの責任分界は追加課題として管理する。")

    doc.add_heading("8. 参照資料", level=1)
    add_bullet(doc, "情報システムPMI_MD基幹統合PJ_TRE_課題管理表.xlsx（最新QA管理表、表内更新日2026-08-28）")
    add_bullet(doc, "PJWest_WMS_Sinops_QA_ReSummary_20260901_QA3_JA.docx（前版日文資料）")
    ...
    add_bullet(doc, "PJWest_情報システム_MD基幹統合PJ _週次定例_20260902.pptx（最新定例会資料）")

    doc.core_properties.title = "WMS・自動補充QA課題再整理サマリ（2026-09-02）"
    doc.core_properties.subject = "最新QA・定例会状況、残課題および設計・開発着手判断"
