...
- 14/15 棚割開始日
- 店舗系 R（24/25）は `SIREJAN_RCMDORDER_LAST` 7 項目。数量は **×1000 しない**（発注単位の整数倍であれば可、倍数へ丸めない）。Layer① は **共通 1 ジョブ**（STEP20 で当日／昨日の翌日以降を 1 本採用）。Ordertype は部門→自動発注GCS `JIHA.BUMON_ZONE`→`JIHA.ZONE_LOGIC`（01→0、02日配/03生鮮→1）。残は店/仕入先桁・最終値化・締切時刻 TBD-19。倉庫系 W（22/23）は G01
