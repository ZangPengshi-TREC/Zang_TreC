...
扱い: **詳細設計の IF 一覧。** 人日は本表に無い。工数は見積 403 のまま。生鮮増分（2026-09-18 会議）は未掲載。  
...
| 層 | 範囲 | 本表の処理内容 |
|---|---|---|
| Layer1 | TRIAL MD基幹 ↔ GCS | 外部 IF。**型変換・フィールド選別・コード変換・編集はここ。** 抽出してファイル生成し GCS へ。受信側は逆 |
| Layer2 | GCS → 西友 DWH | DataSpider Intake。**マスタのみ。全フィールド通過（型変換・項目選別なし）。** トランは DWH を通さず Layer3 へ |
| Layer3 | DWH・GCS ↔ 業務 | DataSpider。**転送・配置のみ（全フィールド）。型変換・選別・コード編集はしない**（Layer1 側）。受信側は取込 |
...
現行アーキテクチャとの差（一覧を優先して IF 有無を書く。経路の細部はアーキテクチャ側）:

- データ元は表上「基幹システム」。抽出は集計 SV／クラウド機、という置き方は `SCM-IFクラウド機アーキテクチャ.md`
- **DSS 加工範囲は 2026-09-21 凍結:** 原則として型変換・フィールド選別はしない（全フィールド）。変換・選別は Layer1。原本 xlsx の Layer3「コード変換・編集」文言は **読み替え**（実装は Layer1）
...
表の「ファイル」列は **論理名**。GCS 上の物理名は必ず `{論理ベース}_YYYYMMDDhhmmss.{拡張子}`（例: 論理 `item_mst.txt` → `item_mst_20260921143000.txt`）。連携用 GCS・自動発注 GCS とも同じ（DD-00）。
...
本作業のプログラム対象（403）は従来どおり自動補充。BO は一覧上の記載。
...
- 22–23 の倉庫勧告レイアウト名（24/25 は §5 で紐付け済）
- 自動発注 GCS 正式桶名（TBD）
...
## 5. 詳細設計書の紐付け（2026-09-22）

原本 xlsx の `01_IF一覧_自動補充Sinops系`「レイアウト」列へ登録済:

| IF | 名称 | 詳細設計書 | Data Flow |
|---|---|---|---|
| Sinops-04 | 仕入先マスタ | `Sinops_IFレイアウト_仕入先マスタ.xlsx`（テンプレ見本） | `templates/データフロー図.drawio` |
| Sinops-24 | 発注勧告（当日分） | `DD/Sinops_IFレイアウト_発注勧告.xlsx` | `DD/Sinops-IF-024_発注勧告_DataFlow.drawio` |
| Sinops-25 | 発注勧告（翌日以降分） | `DD/Sinops_IFレイアウト_発注勧告翌日以降.xlsx` | `DD/Sinops-IF-025_発注勧告翌日以降_DataFlow.drawio` |
